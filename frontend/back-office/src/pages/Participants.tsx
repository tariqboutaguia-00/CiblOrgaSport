...
export default function Participants() {
  ...
  const columns = useMemo(() => {
    ...
    const firstItemKeys = Object.keys(participants[0]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-sm font-medium text-brand-600">Participants</p>
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white">
          Liste des participants
        </h2>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
          Cette page affiche les participants récupérés depuis le backend.
        </p>
      </div>
                ...
                {participants.map((participant, index) => (
                  ...
                  >
                    {columns.map((column) => (
                      ...
                      >
                        {formatValue(participant[column])}
                      </td>
                    ))}
                  </tr>
                ))}
